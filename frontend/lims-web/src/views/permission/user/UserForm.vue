...
import { UserPostData } from "@/api/types/user";
...
const rolesDict = ref<[IDict] | []>([]);
const formData = ref<UserPostData>({
  username: "",
  pwd: "",
  confPwd: "",
  roles: [] as number[],
...
const loadRoles = async () => {
  ...
  rolesDict.value = data.data.list.map((item) => ({
    code: item.id,
    name: item.name,
  })) as [IDict];
};
const loadUser = async () => {
  ...
  const data = await getUser(props.adminId);
  formData.value = data.data;
};
const handleDialogOpen = () => {
  formLoading.value = true;
  Promise.all([loadRoles(), loadUser()]).finally(() => {
...
const handleSubmit = async () => {
  ...
  if (props.adminId) {
    await updateUser(props.adminId, formData.value);
  } else {
